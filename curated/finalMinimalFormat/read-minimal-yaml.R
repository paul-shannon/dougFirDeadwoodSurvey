...
valid <- function(id, el)
{
   ...
   checkTrue(is.character(failureMode) & failureMode %in% c("NA", "uproot", "roots", "stump", "snag", "logging"))
...
if(valid(names(x)[1], x[[1]])){
   ...
   tsv.filename <- sprintf("tsv/%s.tsv", base.name)
   write.table(tbl, file=tsv.filename, sep="\t", row.names=FALSE, quote=FALSE)
   }
