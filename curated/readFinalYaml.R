...
options(digits=9)
base.name <- "logs-with-deathDates-final"
...
valid <- function(id, el)
{
   ...
   notes <- el[[ "notes"]]
   printf("--- notes valid: %s", nchar(notes) > 0)
   status <- el[[ "status"]]
   printf("--- status valid: %s", status %in% c("placeholder", "needs work", "ready"))
   ...
   dbh <- el[[ "dbh"]]
   printf("--- dbh valid: %s", is.numeric(dbh) & dbh > 8 & dbh < 85)
   ...
   printf("--- yearOfDeath.evidence valid: %s", nchar(yearOfDeath.evidence) > 0)

   failureMode <- el[[ "failureMode"]]
   printf("failureMode valid: %s", is.character(failureMode) & failureMode %in% c("uproot", "roots", "snag"))
   failureHeight <- el[[ "failureHeight"]]
   printf("failureHeight valid: %s", is.numeric(failureHeight) & failureHeight %in% 0:100)
   directionOfFall <- el[[ "directionOfFall"]]
   printf("directionOfFall valid: %s", is.numeric(directionOfFall) & directionOfFall %in% 0:360)
   branches.small <- el[[ "branches.small"]]
   printf("branches.small valid: %s", is.numeric(branches.small) & branches.small %in% 0:40)

   branches.large <- el[[ "branches.large"]]
   printf("branches.large valid: %s", is.numeric(branches.large) & branches.large %in% 0:40)

   branches.huge <- el[[ "branches.huge"]]
   printf("branches.huge valid: %s", is.numeric(branches.huge) & branches.huge %in% 0:40)

   ground.contact <- el[[ "ground.contact"]]
   printf("ground.contact valid: %s", is.numeric(ground.contact) &
                                      ground.contact >= 0 & ground.contact <= 1)

   sag <- el[[ "sag"]]
   printf("sag valid: %s", is.numeric(sag) & sag >= 0 & sag <- 1)

   bark <- el[[ "bark"]]
   printf("bark valid: %s", is.numeric(bark) & bark >= 0 & bark <= 1)

   bottom.decay <- el[[ "bottom.decay"]]
   printf("bottom.decay valid: %s", is.numeric(bottom.decay) & bottom.decay %in% 0:360)
   ...
   printf("mid.decay valid: %s", is.numeric(mid.decay) & mid.decay >= 0 & mid.decay <= 1)

   mid.decay.notes <- el[[ "mid.decay.notes"]]
   printf("mid.decay.notes valid: %s", is.character(mid.decay.notes))

   top.decay <- el[[ "top.decay"]]
   printf("top.decay valid: %s", is.numeric(top.decay) & top.decay >= 0 & top.decay <= 1)

   top.decay.length <- el[[ "top.decay.length"]]
   printf("top.decay.length valid: %s", is.numeric(top.decay.length) & top.decay.length %in% 0:100)
   ...
   printf("bottom.corss.section valid: %s",
          is.character(bottom.cross.section) & bottom.cross.section %in% c("round", "oval", "depleted"))

   top.cross.section <- el[[ "top.cross.section"]]
   printf("top.corss.section valid: %s",
          is.character(top.cross.section) & top.cross.section %in% c("round", "oval", "depleted"))

   epiphytes.meter.1 <- el[[ "epiphytes.meter.1"]]
   moss <- epiphytes.meter.1$moss
   printf("ephiphytes.meter.1")
   printf("     moss valid: %s", is.numeric(moss) & moss >= 0 & moss <= 1.0)
   leafCoverage <- epiphytes.meter.1$leafCoverage
   printf("     leafCoverage valid: %s", is.numeric(leafCoverage) & leafCoverage >= 0 & leafCoverage <= 1.0)
   ...
   known.species <- c("psme", "tshe", "mane", "pomu", "hodi", "lohi", "syal",
                      "syhe", "thpi", "arme", "vaov", "vapa", "gash", "ruur",
                      "qual", "quve", "quru", "quga", "acma")

   printf("epiphyte.species valid: %s",  all(species %in%  known.species))

   epiphytes.overall.moss <- el[[ "epiphytes.overall.moss"]]
   printf("epiphytes.overall.moss valid: %s", is.numeric(epiphytes.overall.moss) &
                                              epiphytes.overall.moss >= 0 &
                                              epiphytes.overall.moss <= 1)
